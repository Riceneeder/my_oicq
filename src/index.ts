...
import cartoonR18 from "./plugins/cartoonR18"
import findAnswer from "./plugins/findAnswer"
import xiaoai from "./plugins/xiaoai"
...
client.on('message.group',async (data) => {
    let type = data.message[0].type;
    if(type == 'at'){
        let msg = data.message[1];
        let _msg = JSON.stringify(msg);
        let _m_sg = JSON.parse(_msg).data.text;
        let msg_back = await xiaoai(_m_sg);
        data.reply(`${msg_back}`);
    }
})